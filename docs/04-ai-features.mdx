...
Built by `data-enricher` and read through `cookie-web-tasks`'s `GET /tasks`:
...
- **News round-up** — new GitHub repos and Product Hunt launches ranked against interests stored in `users.prefs` (edited in Settings → Personalisation), plus an unfiltered ten-story headline list with up to three recent West Lothian stories from Edinburgh Live and the remaining places filled by BBC UK.

`POST /tasks/refresh` calls `data-enricher`'s `POST /run?phase=today` over the `ENRICHER` service binding, carrying the shared trigger token and enforcing the configured owner before the rebuild.
