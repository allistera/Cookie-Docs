...
| --- | --- | --- |
| Static SPA + legacy send | **Vercel** | `mail.infinitywave.online` — the Vue app and `/api/send`, still used by the current web and iOS clients |
...
| `cookie-web-send` | **Cloudflare** | `send-api.infinitywave.online` — outbound/scheduled delivery; currently used by the flusher service binding while web/iOS retain Vercel compatibility calls |
...
The browser and iOS app never talk to Postgres directly. Both authenticate with Auth0. The browser calls the ten Cloudflare API Workers cross-origin and still uses Vercel's compatibility send route; the current iOS build uses a mix of Worker endpoints and legacy Vercel paths:
...
```mermaid
flowchart TB
    Inputs["05:00 UTC cron"] --> Enricher["data-enricher"]
...
flowchart TB
    ...
    Flusher --> API["cookie-web-send<br/>service binding"]
    API --> DB[("Supabase Postgres")]
    API --> Resend[["Resend"]]
```

- **`data-enricher`** runs daily at 05:00 UTC (plus on-demand via `POST /run`). It runs AI task analysis over important mail, triages the last 24 hours of Inbox mail, and assembles the personalised news round-up — all written to Postgres and read by Cookie-Web's AI Today page through the `cookie-web-tasks` Worker.
- **`scheduled-send-flusher`** runs every 5 minutes and calls `POST /send/flush` on `cookie-web-send` over a Cloudflare service binding. It owns no mail-sending logic itself — `cookie-web-send` claims the due rows and calls Resend.
