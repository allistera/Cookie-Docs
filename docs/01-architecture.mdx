...
| --- | --- | --- |
| Static SPA + 9 Functions | **Vercel** | `mail.infinitywave.online` — the Vue app, plus `api/`: `emails`, `search`, `ask`, `compose`, `send`, `summarize`, `calendar-events`, `read-receipts`, `notification-event` |
...
flowchart TB
    ...
    subgraph Supabase["Supabase"]
        DB[("Postgres + pgvector<br/>messages, threads, attachments,<br/>rules and message_ai")]
    end
...
flowchart LR
    subgraph CF["Cloudflare"]
        ...
        Apply["8 · Validate + apply<br/>labels, spam, priority, vector"]
    ...
    subgraph OpenAI["OpenAI"]
        AI["7 · Structured classification<br/>+ 1,536-dimension embedding"]
    end

    subgraph Supabase["Supabase"]
        DB[("Postgres + pgvector<br/>accepted AI state")]
    end
...
flowchart TB
    ...
    subgraph Vercel["Vercel"]
        API["Cookie-Web API<br/>emails, search, ask, compose,<br/>send, summarize, calendar"]
    end

    subgraph CF["Cloudflare"]
        Workers["cookie-web-labels<br/>cookie-web-messages<br/>cookie-web-tasks"]
    end
